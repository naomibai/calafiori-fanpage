...
GitHub Actions (每 6 小时 / 手动触发)
  ├─ npm run fetch-fixtures → ESPN 公开接口（无需密钥）→ data/fixtures.json
...
赛程采集使用 ESPN 公开接口，无需任何密钥。新闻标题中文翻译需要设置一个 Secret（Settings → Secrets and variables → Actions）：
...
4. 脚本抓取各账号最近动态（链接/文案/图片/日期）→ 合并进 `data/social.json`（按链接去重、**只保留最近 7 天**、上限 60 条）
